...
const STORAGE_KEY = "led-matrix-studio-cache";
...
if (typeof window !== "undefined") {
    ...
    if (saved) {
        try {
            ...
            if (parsed.frames) frames.value = parsed.frames;
            if (parsed.currentFrameIndex !== undefined)
                currentFrameIndex.value = parsed.currentFrameIndex;
            if (parsed.selectedColor) selectedColor.value = parsed.selectedColor;
            if (parsed.selectedIntensity !== undefined)
                selectedIntensity.value = parsed.selectedIntensity;
            if (parsed.fps !== undefined) fps.value = parsed.fps;
        } catch (e) {
    ...
    watch(
        [frames, currentFrameIndex, selectedColor, selectedIntensity, fps],
        () => {
...
export const applyIntensity = (hexColor: string, intensityPercent: number) => {
    if (hexColor === "#000000" || hexColor.toLowerCase() === "#000")
        return hexColor;
...
export const useMatrixStore = () => {
    ...
    const applyTool = (row: number, col: number) => {
        ...
        const activeColor = applyIntensity(
            selectedColor.value,
            selectedIntensity.value,
        );

        if (currentTool.value === "fill") {
            floodFill(row, col, activeColor);
            return;
        }

        const colorToApply =
            currentTool.value === "erase" ? DEFAULT_COLOR : activeColor;
    ...
    const fillGridInternal = (color: string) => {
        frames.value[currentFrameIndex.value] = frames.value[
    ...
    const floodFill = (startRow: number, startCol: number, newColor: string) => {
        ...
        const targetColor = frame[startRow][startCol];
        ...
        while (stack.length > 0) {
            ...
            if (frame[r][c] !== targetColor) continue;
    ...
    const moveFrame = (fromIndex: number, toIndex: number) => {
        if (
            fromIndex >= 0 &&
            fromIndex < frames.value.length &&
            toIndex >= 0 &&
            toIndex < frames.value.length &&
            fromIndex !== toIndex
        ) {
